...
async def google(message: Message):
    ...
    service = build("customsearch", "v1", developerKey="AIzaSyCkFJGlkkvPRkNpq5A1ioFTjgqwgFFrOFU")
    query = query.replace(' ', '+')
    if not Config.SILENT:
        message = await message.edit(lang('google_processing'))
    results = ""
    for i in service.cse().list(q=query, cx="YOUR_SEARCH_ENGINE_ID").execute()['items']:
        try:
            title = i['title'][:30] + '...'
            link = i['link']
            results += f"\n<a href=\"{link}\">{title}</a> \n"
